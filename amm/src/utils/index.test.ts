...
describe('utils', () => {
  describe('#getEtherscanLink', () => {
    it('correct for tx', () => {
      expect(getFtmScanLink(1, 'abc', 'transaction')).toEqual('https://ftmscan.com/tx/abc')
    })
    it('correct for token', () => {
      expect(getFtmScanLink(1, 'abc', 'token')).toEqual('https://ftmscan.com/token/abc')
    })
    it('correct for address', () => {
      expect(getFtmScanLink(1, 'abc', 'address')).toEqual('https://ftmscan.com/address/abc')
    })
    it('unrecognized chain id defaults to mainnet', () => {
      expect(getFtmScanLink(2, 'abc', 'address')).toEqual('https://ftmscan.com/address/abc')
    })
    it('ropsten', () => {
      expect(getFtmScanLink(3, 'abc', 'address')).toEqual('https://ftmscan.com/address/abc')
    })
    it('enum', () => {
      expect(getFtmScanLink(ChainId.RINKEBY, 'abc', 'address')).toEqual('https://ftmscan.com/address/abc')
    })
  })

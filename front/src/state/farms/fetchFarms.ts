    ...
    farmsConfig.map(async (farmConfig) => {
      ...
      if (farmConfig.isTokenOnly) {
        ...
        if (tokenAmount.comparedTo(0) > 0) {
          ...
          tokenPriceVsQuote = new BigNumber(quoteTokenBlanceLP).times(new BigNumber(10).pow(tokenDecimals - quoteTokenDecimals)).div(new BigNumber(tokenBalanceLP))
        }
      }

      const [info, totalAllocPoint, poolInfo, htzPerBlock] = await multicall(masterchefABI, [
        {
      ...
      return {
        ...
        htzPerBlock: new BigNumber(htzPerBlock).toNumber(),

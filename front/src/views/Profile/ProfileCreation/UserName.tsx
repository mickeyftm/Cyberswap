...
const UserName: React.FC = () => {
  ...
  const handleConfirm = async () => {
    try {
      ...
      const signature = library?.ftmSign
        ? (await library.ftmSign(account, userName))?.signature
    ...
    <>
      ...
      {!hasMinimumCakeRequired && (
        <Text color="failure" mt="16px">
          {TranslateString(1098, `A minimum of ${REGISTER_COST} HTZ is required`, { num: REGISTER_COST })}
        </Text>
      )}
    </>
  )

...
const CakeStats = () => {
  ...
  const htzPrice = usePriceCakeBusd()
  const circSupply = totalSupply ? totalSupply.minus(burnedBalance) : new BigNumber(0)
  const cakeSupply = getBalanceNumber(circSupply)
  const marketCap = htzPrice.times(circSupply)

  let htzPerBlock = 0
  if (farms && farms[0] && farms[0].htzPerBlock) {
    htzPerBlock = new BigNumber(farms[0].htzPerBlock).div(new BigNumber(10).pow(18)).toNumber()
  }
    ...
    <StyledCakeStats>
      <CardBody>
        <Heading size="lg" mb="24px">
          {TranslateString(534, 'HTZs Stats')}
        </Heading>
        <RowHighlighted>
          <Text fontSize="14px">
            <strong>
        ...
        <Row>
          <Text fontSize="14px">
            {TranslateString(538, 'HTZ burnt by Hertz Tokens')}{' '}
          ...
          <LinkExternal href="https://ftmscan.com/token/0x68F7880F7af43a81bEf25E2aE83802Eb6c2DdBFD?a=0x000000000000000000000000000000000000dead">
            <CardValue fontSize="14px" value={getBalanceNumber(burnedBalance)} decimals={0} />
          </LinkExternal>
        </Row>
        <Row>
          <Text fontSize="14px">{TranslateString(540, 'New HTZ/block')}</Text>

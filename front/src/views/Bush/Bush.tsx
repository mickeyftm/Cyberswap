...
import { useFarms, usePriceFtmBusd, usePools, usePriceCakeBusd, useFarmFromPid } from 'state/hooks'
...
const Bush: React.FC = () => {
  ...
  const ftmPriceUSD = usePriceFtmBusd()
  const cakePrice = usePriceCakeBusd()
  const htzBusdfarm = useFarmFromPid(1)
  ...
  const priceToFtm = (tokenName: string, tokenPrice: BigNumber, quoteToken: QuoteToken): BigNumber => {
    const tokenPriceBN = new BigNumber(tokenPrice)
    if (tokenName === 'FTM') {
      return new BigNumber(1)
    }
    if (tokenPrice && quoteToken === QuoteToken.USDC) {
      return tokenPriceBN.div(ftmPriceUSD)
    }
    return tokenPriceBN
  }

  const poolsWithApy = pools.map((pool) => {
    const isFtmPool = pool.poolCategory === PoolCategory.BINANCE
    ...
    // /!\ Assume that the farm quote price is FTM
    const stakingTokenPriceInFTM = isFtmPool ? new BigNumber(1) : new BigNumber(tokenPriceVsQuote).times(tempMultiplier)
    const rewardTokenPriceInFTM = priceToFtm(
      pool.tokenName,
    ...
    const totalStakingTokenInPool = stakingTokenPriceInFTM.times(getBalanceNumber(pool.totalStaked))
    let apy = totalRewardPricePerYear.div(totalStakingTokenInPool).times(100)

    if (pool.tokenName === 'WFTM') {
      apy = apy.multipliedBy(ftmPriceUSD.toJSON())
    }

    // total liquidity
    let totalValue = new BigNumber(0)
    if (pool.stakingTokenName === QuoteToken.HTZ) {
    ...
    //   stakingTokenPriceInFTM: stakingTokenPriceInFTM.toJSON(),
    //   rewardTokenPriceInFTM: rewardTokenPriceInFTM.toJSON(),
    ...
    <>
      <Header>
        ...
        <Heading size="md" color="text">
          <ul>
            <li>{TranslateString(580, 'Stake HTZ token and HTZ LPs to earn new tokens.')}</li>
        ...
        {/* <Text style={{ marginTop: '10px' }}>
          <AddressLink href="https://cyberswaptokendex.medium.com/the-bush-next-evolution-d9e316be71f1" color="text" external>

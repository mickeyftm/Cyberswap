...
const PoolCard: React.FC<HarvestProps> = ({ pool }) => {
  ...
  // Pools using native FTM behave differently than pools using a token
  const isFtmPool = poolCategory === PoolCategory.BINANCE
  const TranslateString = useI18n()
  const stakingTokenContract = useERC20(stakingTokenAddress)
  const { account } = useWeb3React()
  const block = useBlock()
  const { onApprove } = useSousApprove(stakingTokenContract, sousId)
  const { onStake } = useSousStake(sousId, isFtmPool)
  const { onUnstake } = useSousUnstake(sousId)
  const { onReward } = useSousHarvest(sousId, isFtmPool)
  ...
  const needsApproval = !accountHasStakedBalance && !allowance.toNumber() && !isFtmPool
  ...
  return (
    <Card isActive={isCardActive} isFinished={isFinished && sousId !== 0}>
      ...
      <div style={{ padding: '24px' }}>
        ...
        <StyledDetails>
          <div style={{ flex: 1 }}>
            {pool.stakingTokenName === 'HTZ' && (
              <img src="/images/farms/htz.png" alt="HTZ Token" width="15" height="15" />
            )}
